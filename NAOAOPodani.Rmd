...
metab_20180_SRF <- metab_diatoms_20171106 %>% 
  pivot_longer(BV9_1:TV9_99) %>% 
  rename(Sample.id = name) %>% 
  inner_join(dat_samp_20180_SRF, by="Sample.id") %>% 
...
temp_vector <- dat_samp_20180_SRF %>%
...
rapa_metab_all <- bind_cols(pairnames, data) %>%
...
```{r warning=FALSE, message=FALSE, cache=TRUE}
ggtern(data = rapa_metab_all, aes(Drel, Rrel, Sjac)) +
  geom_point(size = 0.8, alpha = 0.25, aes(col = `Delta temperature`)) +
...
rapa_metab_all2 <- rapa_metab_all %>% 
...
ggtern(data = rapa_metab_all2, aes(Drel, Rrel, Sjac)) +
  geom_point(size = 0.8, alpha = 0.25, aes(col = `Marine Biome`)) +
...
#/home/lcampese/Data/new_taxonomy/metaG
#ls *.metaG.diatoms | while read line; do sed -e '1d' -e 's/$/ '$line'/g' "$line" >> metaG_diatoms_all_presabs.ssv; done
#lcampese@falkor-frontend:~/Data/new_taxonomy/metaG$ sed 's/ \([0-9]\{,3\}\)SUR[01234]QQSS1[12345].metaG.diatoms/\t\1/g' metaG_diatoms_all_presabs.ssv > metaG_diatoms_all_presabs_v2.ssv
#lcampese@falkor-frontend:~/Data/new_taxonomy/metaG$


#metaG <- fread("metaG_diatoms_all_presabs_v2.ssv") %>%
#  separate(V2, into = c("value", "Station"), sep = "\\s") %>% 
#  rename(geneid = V1) %>% 
#  select(geneid, Station)
...
tara_pabs <- metaG %>% 
  add_count(geneid, name = "Occurrences") %>% 
  filter(Occurrences > thsh) %>% 
  select(geneid, Station)

comn <- tara_pabs %>%
  group_by(Station) %>%
  summarise(data = list(geneid))

pairnames <- expand.grid(S1 = as.character(comn$Station), S2 = as.character(comn$Station))
data      <- expand.grid(D1 = comn$data, D2 = comn$data)


rapa <- bind_cols(pairnames, data) %>%
  mutate(int = map2(D1, D2, ~intersect(.x, .y)),
         a = map_int(int, ~length(.x)),                                                                
         b = map2_int(D1, int, ~length(setdiff(.x, .y))),
         c = map2_int(D2, int, ~length(setdiff(.x, .y))), n = a + b + c) %>%
...
<div class="column-left">
```{r warning=FALSE, message=FALSE, cache=TRUE}
ggtern(data = rapa_metag_all, aes(Drel, Rrel, Sjac)) +
  geom_point(size = 0.8, alpha = 0.25, aes(col = `Delta temperature`)) +
  scale_color_viridis_c() +
  theme_minimal() +
...
#/home/lcampese/Data/new_taxonomy/metaT
# *.metaT.diatoms | while read line; do sed -e '1d' -e 's/$/ '$line'/g' "$line" >> metaT_diatoms_all_presabs.ssv; done
# 's/ \([0-9]\{,3\}\)SUR[01234]QQSS1[12345].metaT.diatoms/\t\1/g' metaT_diatoms_all_presabs.ssv > metaT_diatoms_all_presabs_v2.ssv
#lcampese@falkor-frontend:~/Data/new_taxonomy/metaT$

metaT <- fread("metaT_diatoms_all_presabs_v2.ssv") %>%
  separate(V2, into = c("value", "Station"), sep = "\\s") %>% 
  rename(geneid = V1) %>% 
  select(geneid, Station)
...
rapa_metat <- bind_cols(pairnames, data) %>%
  mutate(int = map2(D1, D2, ~intersect(.x, .y)),
         a = map_int(int, ~length(.x)),                                                                
         b = map2_int(D1, int, ~length(setdiff(.x, .y))),
         c = map2_int(D2, int, ~length(setdiff(.x, .y))), n = a + b + c) %>%
...
<div class="column-left">
```{r warning=FALSE, message=FALSE, cache=TRUE}
ggtern(data = rapa_metat_all, aes(Drel, Rrel, Sjac)) +
  geom_point(size = 0.8, alpha = 0.25, aes(col = `Delta temperature`)) +
  scale_color_viridis_c() +
  theme_minimal() +
...
## Part Two: Zoom in the Transition from Atlantic to Arctic Ocean
...
ggtern(data = rapa_metaB_NAOAO2, aes(Drel, Rrel, Sjac)) +
  geom_point(size = 1, alpha = 0.25, aes(col = `Ocean Region`)) +
  guides(colour = guide_legend(override.aes = list(size=3))) +
  theme_minimal() +
  labs(title = "Presence-absence data patterns in metaB",
       subtitle = paste("Considering OTUs appearing at least in",
